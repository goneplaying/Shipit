...
struct ExchangeMapPage: View {
    ...
    @State private var startCoordinate: CLLocationCoordinate2D? // Start coordinate for POI marker
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Initialize the first route immediately
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                if !filteredShipments.isEmpty && activeCardIndex == -1 {
                    updateActiveCardAndRoute()
                }
            }
        }
    ...
    private func updateActiveCardAndRoute() {
        guard !filteredShipments.isEmpty else { return }
        
        // On first load, use the first shipment (index 0)
        let targetIndex = activeCardIndex == -1 ? 0 : activeCardIndex
        
        if targetIndex >= 0 && targetIndex < filteredShipments.count {
            let activeShipment = filteredShipments[targetIndex]
    ...
    private func initializeFirstRouteIfNeeded() {
        guard !hasInitializedRoute else { return }
        let shipments = filteredShipments
        if !shipments.isEmpty && activeCardIndex == -1 {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                if !self.filteredShipments.isEmpty && self.activeCardIndex == -1 {
                    self.updateActiveCardAndRoute()
                }
            }
        }
    }
    ...
    private func updateRouteForShipment(_ shipment: ShipmentData) {
        ...
        geocodeAddress(shipment.pickupLocation, requestId: requestId) { [requestId] startCoord in
            ...
            // Set start coordinate for POI marker
            DispatchQueue.main.async {
                if self.currentRouteRequestId == requestId {
